...
def save_example_traces_figs(fig, ephys_traces, type):
    ...
    fig.write_image(
        os.path.join(FileSettings.PRESENTATION_FIGURES_FOLDER, filename)
    )
...
def plot_example_GC_traces(traces_dict):
    ...
    for count, (cell, info) in enumerate(traces_dict.items()):
        ...
        fig.add_annotation(
            x=850,
            y=-27 if cell == "GC cell-attached" else -220,
            text="100 ms",
            showarrow=False,
            font=dict(size=30),
            row=1,
        ...
        fig.add_annotation(
            x=990 if cell == "GC cell-attached" else 1010,
    ...
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",
        font_family="Arial",
        legend=dict(font=dict(family="Arial", size=36)),
        width=1200,
        height=300,
    )
...
def save_fig_to_png(fig, legend, rows, cols, png_filename, extra_bottom=False):
    ...
    fig.for_each_annotation(
        lambda a: a.update(font=dict(family="Arial", size=36))
    )

    fig.write_image(
        os.path.join(FileSettings.PRESENTATION_FIGURES_FOLDER, png_filename)
    )
